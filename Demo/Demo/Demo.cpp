...
clock_t t1, t2;
static int sleepTime = 0, second = 0, minute = 0, restart = 0, endGame = 0, showanswer = 0, Errorexit = 0;
...
public:
	GameScene();
	bool isCorrect() //检查是否正确
	{
		for(int i=0; i<9; i++)
			for (int j = 0; j < 9; j++)
			{
				if (MapMain[i][j] != algorithm.tempMainMap[i][j])
					return false;
			}
		return true;
	}
